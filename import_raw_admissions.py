"""将 CSV 批量导入 raw_admission_records。

运行命令：
    python import_raw_admissions.py

重要说明：
    本脚本只把 CSV 数据写入 raw_admission_records，状态默认为 pending。
...
def read_csv_rows() -> list[dict[str, str]]:
    if not CSV_PATH.exists():
        raise FileNotFoundError(f"找不到原始招生数据 CSV：{CSV_PATH}")
...
def get_or_create_raw_data_source(db, row: dict[str, str]) -> tuple[int, bool]:
...
def main() -> None:
    ...
    try:
        for index, row in enumerate(rows, start=2):
            try:
                ...
                data = build_raw_record_data(row, raw_source_id, index)
                ...
                skipped_errors += 1
                error_messages.append(f"第 {index} 行导入失败：{exc}")
    ...
    print(f"跳过错误数据数量：{skipped_errors}")
